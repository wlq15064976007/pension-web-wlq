<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="联系人" min-width="100px">
        <template slot-scope="scope">{{ scope.row.linkMan }}</template>
      </el-table-column>
      <el-table-column label="联系人电话" min-width="100px">
        <template slot-scope="scope">{{ scope.row.linkPhone }}</template>
      </el-table-column>
      <el-table-column label="邮箱" min-width="100px">
    ...
    <!-- 表单 -->
    <orgForm ref="fromOrg" @updateRow="updateRow" />

  </div>
</template>

<script>
import { findPage, del, edit, delAll } from '@/api/org'
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      this.$refs.fromOrg.handleCreate()
    },
    handleUpdate(index, id) {
      this.currentIndex = index
      this.$refs.fromOrg.handleUpdate(id)
    },
    updateRow(temp) {
