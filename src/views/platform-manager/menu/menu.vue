<template>
  <div class="app-container">
    ...
    >
      ...
      <vxe-table-column field="type" title="类型" :formatter="formatType" />
      <vxe-table-column field="permission" title="权限标志" />
    ...
    <!-- 表单 -->
    <menuForm ref="fromMenu" :menu-list="list" @updateRow="updateRow" />
  </div>
</template>

<script>
import { findByModule, del, change, delBatch, changeAll } from '@/api/menu'
...
export default {
  ...
  methods: {
    ...
    getModules() {
      findAll().then(response => {
        ...
        if (this.tabs.length > 0) {
          this.moduleId = this.tabs[0].id
          this.tabActive = this.moduleId
        }
        this.fetchList()
      })
    },
    fetchList() {
      ...
      findByModule(this.moduleId).then(response => {
    ...
    handleCreate() {
      this.$refs.fromMenu.handleCreate()
    },
    handleUpdate(id) {
      this.$refs.fromMenu.handleUpdate(id)
    },
