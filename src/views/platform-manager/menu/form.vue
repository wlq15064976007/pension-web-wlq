  ...
  <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
    <el-form ref="dataForm" :roles="roles" :model="temp" label-position="left" label-width="70px" style="width: 100%; height: 50vh;overflow-y: scroll;">
      <el-form-item label="上级">
        <treeselect v-model="temp.parentId" :multiple="false" :options="getMenuList" />
      </el-form-item>
      ...
      <el-form-item label="描述">
        <el-input v-model="temp.describe" clearable />
      </el-form-item>
    </el-form>
...
<script>
import { getinfo, save } from '@/api/menu'
...
export default {
  ...
  computed: {
    getMenuList() {
      return this.menuTop.concat(tree.listToTreeSelect(this.menuList))
    }
  },
  watch: {
    ...
    handleUpdate(id) {
      ...
      getinfo(id).then(response => {
        if (response.code === 20000) {
          ...
          _this.temp.type = response.data.type
          _this.temp.url = response.data.url
          _this.temp.permission = response.data.permission
          _this.temp.enabled = response.data.enabled
          _this.temp.sort = response.data.sort
          _this.temp.describe = response.data.describe
          _this.pid = tree.getParentsId(this.menuList, id)
        }
      })
      this.$nextTick(() => {
        this.$refs['dataForm'].clearValidate()
