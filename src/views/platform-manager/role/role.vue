<template>
  <div class="app-container">
    <el-row style="margin-bottom:2vh">
      <el-select v-model="listQuery.orgId" placeholder="请选择" @change="orgChange">
    ...
    <!-- 表单 -->
    <roleForm ref="formRole" :org-id="listQuery.orgId" @updateRow="updateRow" />

  </div>
</template>

<script>
import { findPage, del, edit, delAll } from '@/api/role'
...
export default {
  ...
  methods: {
    getOrgs() {
      findAll().then(response => {
        ...
        if (this.orgs.length > 0) {
          this.listQuery.orgId = this.orgs[0].id
        }
        this.fetchList()
      })
    },
    ...
    handleCreate() {
      this.$refs.formRole.handleCreate()
    },
    handleUpdate(index, id) {
      this.currentIndex = index
      this.$refs.formRole.handleUpdate(id)
    },
    updateRow(temp) {
