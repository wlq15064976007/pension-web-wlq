<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="欠费" min-width="90px">
        <template slot-scope="scope">
          {{ scope.row.arrear }}
        </template>
      </el-table-column>
    ...
    fetchList() {
      ...
      findCheckinPage(this.listQuery).then(response => {
        ...
        if (this.list && this.list.length > 0) {
          this.list[0].arrear = -340
        }
        if (this.list && this.list.length > 1) {
          this.list[1].arrear = -150
        }
        if (this.list && this.list.length > 2) {
          this.list[2].arrear = -570
        }
        if (this.list && this.list.length > 3) {
          this.list[3].arrear = -280
        }
        if (this.list && this.list.length > 4) {
          this.list[4].arrear = -320
        }
        if (this.list && this.list.length > 5) {
          this.list[5].arrear = -30
        }
        if (this.list && this.list.length > 6) {
          this.list[6].arrear = -265
        }
        if (this.list && this.list.length > 7) {
          this.list[7].arrear = -11
        }
        if (this.list && this.list.length > 8) {
          this.list[8].arrear = -56
        }
        if (this.list && this.list.length > 9) {
          this.list[9].arrear = -78
        }
        if (this.list && this.list.length > 10) {
          this.list[10].arrear = -20
        }
