<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="捐赠时间" width="160px" align="center">
        <template slot-scope="scope">{{ scope.row.donationTime | formatDate }}</template>
      </el-table-column>
      <el-table-column label="捐赠人或单位" min-width="110px">
        <template slot-scope="scope">{{ scope.row.donationPeopleUnit }}</template>
      </el-table-column>
      <el-table-column label="联系电话" min-width="100px">
        <template slot-scope="scope">{{ scope.row.linkPhone }}</template>
      </el-table-column>
      <el-table-column label="捐赠地点" min-width="100px">
        <template slot-scope="scope">{{ scope.row.donationPlace }}</template>
      </el-table-column>
      <el-table-column label="捐赠物品" min-width="150px">
        <template slot-scope="scope">{{ scope.row.donationGoods }}</template>
      </el-table-column>
      <el-table-column label="数量" min-width="50px">
        <template slot-scope="scope">{{ scope.row.donationQuantity }}</template>
      </el-table-column>
      <el-table-column label="捐赠负责人" width="120px" align="center">
        <template slot-scope="scope">{{ scope.row.donationHead }}</template>
      </el-table-column>
      <el-table-column label="分发方式" width="100px" align="center">
        <template slot-scope="scope">{{ scope.row.distributeWay }}</template>
      </el-table-column>
      <el-table-column label="分发人" width="80px" align="center">
        <template slot-scope="scope">{{ scope.row.distributePeople }}</template>
      </el-table-column>
    ...
    <!-- 表单 -->
    <donationForm ref="formModel" @updateRow="updateRow" />

  </div>
</template>

<script>
import { findPage, del, edit, delAll } from '@/api/donation'
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      this.$refs.formModel.handleCreate()
    },
    handleUpdate(index, id) {
      this.currentIndex = index
      this.$refs.formModel.handleUpdate(id)
    },
    updateRow(temp) {
