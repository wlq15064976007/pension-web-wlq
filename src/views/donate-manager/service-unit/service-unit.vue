<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="单位名称" min-width="100px">
        <template slot-scope="scope">
          {{ scope.row.unitName }}
        </template>
      </el-table-column>
      <el-table-column label="参加人数" min-width="100px">
        <template slot-scope="scope">{{ scope.row.joinPeoples }}</template>
      </el-table-column>
      <el-table-column label="服务内容" min-width="100px">
        <template slot-scope="scope">{{ scope.row.serviceContent }}</template>
      </el-table-column>
      <el-table-column label="负责人" min-width="100px">
        <template slot-scope="scope">{{ scope.row.headAutograph }}</template>
      </el-table-column>
      <el-table-column label="联系电话" min-width="100px">
        <template slot-scope="scope">{{ scope.row.linkPhone }}</template>
      </el-table-column>
      <el-table-column label="活动时间" width="160px" align="center">
        <template slot-scope="scope">{{ scope.row.serviceTime | formatDate }}</template>
      </el-table-column>
    ...
    <!-- 表单 -->
    <serviceUnitForm ref="formModel" @updateRow="updateRow" />

  </div>
</template>

<script>
import { findPage, del, edit, delAll } from '@/api/service-unit'
import waves from '@/directive/waves'
import { pickerOptions, getArrByKey } from '@/utils'
import serviceUnitForm from './form'

export default {
  name: 'ServiceUnit',
  components: { serviceUnitForm },
  directives: {
    waves
  },
  filters: {
