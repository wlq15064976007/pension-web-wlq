<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="单位/学校" min-width="100px">
        <template slot-scope="scope">{{ scope.row.unitSchool }}</template>
      </el-table-column>
      <el-table-column label="服务内容" min-width="150px">
        <template slot-scope="scope">{{ scope.row.serviceContent }}</template>
      </el-table-column>
      <el-table-column label="是否培训" width="50px" align="center">
        <template slot-scope="scope">{{ scope.row.isTrain | trainFilter }}</template>
      </el-table-column>
      <el-table-column label="起始时间" width="160px" align="center">
    ...
    <!-- 表单 -->
    <servicePersonalForm ref="formModel" @updateRow="updateRow" />

  </div>
</template>

<script>
import { findPage, del, edit, delAll } from '@/api/service-personal'
import waves from '@/directive/waves'
import { pickerOptions, getArrByKey } from '@/utils'
import servicePersonalForm from './form'

export default {
  name: 'ServicePersonal',
  components: { servicePersonalForm },
  directives: {
    waves
  },
  filters: {
